...
function applyCharacterModelTransform(model, value = {}) {
  ...
  if (Number.isFinite(value.modelGroundClearance)) {
    ...
    const min = new three.Box3().setFromObject(model).min.y;
...
function createShadowPlane(scene, options = {}) {
  const plane = new three.Mesh(new three.PlaneGeometry(options.size ?? 20, options.size ?? 20), new three.ShadowMaterial({ opacity: options.opacity ?? 0.3 }));
...
function createDefaultCharacterLighting(scene, renderer, initial = {}) {
  const ambient = new three.HemisphereLight(16251903, 7041664, 0);
  const key = new three.DirectionalLight(16776180, 0);
  const fill = new three.DirectionalLight(15266047, 0);
  const rim = new three.DirectionalLight(14543103, 0);
  ...
  const rebuild = () => {
    ...
    const room = new RoomEnvironment_js.RoomEnvironment();
    try {
  ...
  const apply = () => {
    ...
    for (const material of plane ? Array.isArray(plane.material) ? plane.material : [plane.material] : []) if (material instanceof three.ShadowMaterial) material.opacity = settings.shadowOpacity;
  };
  rebuild();
  apply();
...
function createDefaultCharacterScene(container, options = {}) {
  ...
  const camera = new three.PerspectiveCamera(options.cameraFov ?? 45, width / height, 0.1, 1e3);
...
var findTrackTarget = (model, track) => {
  try {
    ...
    const object = key ? model.getObjectByProperty("uuid", key) ?? three.PropertyBinding.findNode(model, key) : model;
  ...
  for (const track of clip.tracks ?? []) {
    ...
    if (kind === "boneTransform" || kind === "objectTransform") {
      ...
      if (property === "rotation" || property === "quaternion") {
        tracks.push(new three.QuaternionKeyframeTrack(`${object.uuid}.quaternion`, times, values));
      } else if (property === "position") {
        tracks.push(new three.VectorKeyframeTrack(`${object.uuid}.position`, times, values));
      } else if (property === "scale") {
        tracks.push(new three.VectorKeyframeTrack(`${object.uuid}.scale`, times, values));
      }
      continue;
    }
    if (kind === "meshVisibility") {
      ...
      if (!mesh) continue;
      tracks.push(new three.NumberKeyframeTrack(`${mesh.uuid}.visible`, times, values));
    }
  }
...
async function load() {
  const resolveAsset = (path) => new URL(path, (typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('index.cjs', document.baseURI).href)));
...
exports.ANNOTATION_CAMERA_FRAMING_STRIDE = ANNOTATION_CAMERA_FRAMING_STRIDE;
exports.CAMERA_FLIGHT_SAMPLE_STRIDE = CAMERA_FLIGHT_SAMPLE_STRIDE;
exports.CHARACTER_SCENE_TYPES = CHARACTER_SCENE_TYPES;
exports.CHARACTER_SCENE_TYPE_IDS = CHARACTER_SCENE_TYPE_IDS;
exports.DEFAULT_CHARACTER_LIGHTING_PRESETS = DEFAULT_CHARACTER_LIGHTING_PRESETS;
exports.DEFAULT_CHARACTER_LIGHTING_PRESET_ID = DEFAULT_CHARACTER_LIGHTING_PRESET_ID;
exports.DEFAULT_CHARACTER_LIGHTING_PRESET_IDS = DEFAULT_CHARACTER_LIGHTING_PRESET_IDS;
exports.DEFAULT_CHARACTER_LIGHTING_SETTINGS = DEFAULT_CHARACTER_LIGHTING_SETTINGS;
exports.DEFAULT_CHARACTER_SCENE_TYPE_ID = DEFAULT_CHARACTER_SCENE_TYPE_ID;
exports.DEFAULT_HAIR_COLOR_APPEARANCE = DEFAULT_HAIR_COLOR_APPEARANCE;
exports.EMBODY_CORE_ABI_VERSION = EMBODY_CORE_ABI_VERSION;
exports.HAIR_COLOR_PRESETS = HAIR_COLOR_PRESETS;
exports.HAIR_CONFIG_STRIDE = HAIR_CONFIG_STRIDE;
exports.HAIR_HEAD_STATE_STRIDE = HAIR_HEAD_STATE_STRIDE;
exports.HAIR_MORPH_OUTPUT_STRIDE = HAIR_MORPH_OUTPUT_STRIDE;
exports.HAIR_STATE_STRIDE = HAIR_STATE_STRIDE;
exports.HUMANOID_SKELETON_TEMPLATES = HUMANOID_SKELETON_TEMPLATES;
exports.JONATHAN_HUMANOID_SKELETON_TEMPLATE = JONATHAN_HUMANOID_SKELETON_TEMPLATE;
exports.MARKER_ENDPOINT_STRIDE = MARKER_ENDPOINT_STRIDE;
exports.MARKER_VISIBILITY_FACTORS_STRIDE = MARKER_VISIBILITY_FACTORS_STRIDE;
exports.MESH_PROPORTIONS_STRIDE = MESH_PROPORTIONS_STRIDE;
exports.PACKED_BONE_FRAME_DELTA_STRIDE = PACKED_BONE_FRAME_DELTA_STRIDE;
exports.PACKED_MORPH_FRAME_DELTA_STRIDE = PACKED_MORPH_FRAME_DELTA_STRIDE;
exports.TEMPLATE_SKELETON_FIT_SOLUTION_STRIDE = TEMPLATE_SKELETON_FIT_SOLUTION_STRIDE;
exports.TEMPLATE_SKELETON_FIT_TRANSFORM_STRIDE = TEMPLATE_SKELETON_FIT_TRANSFORM_STRIDE;
exports.THREE_BLENDING_MODES = THREE_BLENDING_MODES;
exports.ThreeFrameApplier = ThreeFrameApplier;
exports.ThreeModelInspector = ThreeModelInspector;
exports.applyCharacterModelTransform = applyCharacterModelTransform;
exports.collectMorphMeshes = collectMorphMeshes;
exports.createAnimationClipFromClipIR = createAnimationClipFromClipIR;
exports.createDefaultCharacterLighting = createDefaultCharacterLighting;
exports.createDefaultCharacterScene = createDefaultCharacterScene;
exports.createShadowPlane = createShadowPlane;
exports.disposeCharacterModel = disposeCharacterModel;
exports.getEmbodyCore = getEmbodyCore;
exports.initEmbodyCore = initEmbodyCore;
exports.loadCharacterModel = loadCharacterModel;
exports.normalizeDefaultCharacterLightingSettings = normalizeDefaultCharacterLightingSettings;
exports.parseCharacterModel = parseCharacterModel;
exports.requireInitializedEmbodyCore = requireInitializedEmbodyCore;
exports.resetEmbodyCoreForTests = resetEmbodyCoreForTests;
exports.serializeAnimationClips = serializeAnimationClips;
